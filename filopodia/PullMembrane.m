...
a = alpha;
a0 = alpha0; % end point of applied deviatoric curvature
gt = t;
lam = lambda*R0^2/k0;
c0 = C0*R0; % deviatoric curvature
g = gamma;
aF=rF;  % area of applied force
aIn = rIn;  % start point applied deviatoric curvature
yp = zp/R0;
iSol = initSol;
...
coatArea = [aIn alpha0];
actArea = [0 aF];
plotTitle = sprintf('Fz = %0.3f pN, D = %0.3f mum^{-1}', f, C0*1000);
...
dXdt = [cos(X(3))/X(1);
        ...
        fbar*cos(X(3))-2*(X(4))*(X(4)^2+(X(4)-sin(X(3))/X(1))^2)+2*X(4)*((X(4))^2+(sin(X(3))/X(1)-X(4)-M)^2+X(6)-2*(sin(X(3))/X(1)-X(4))*(sin(X(3))/X(1)-X(4)-M))-2*cos(X(3))/X(1)*(-1/2*X(5)/X(1)+1*X(4)*cos(X(3))/X(1)-1*sin(X(3))*cos(X(3))/X(1)^2-1/2*dM*X(1));
        2*(sin(X(3))/X(1)-X(4)-M)*dM- fbar*sin(X(3))/X(1)];
