...
function [FvsZp, PullSol, compZpRng] = loop(alpha, lambda, acoat, rF, rIn, zpRng, f0, k0, gamma, C0, R0, initSol)
mesh=(0:0.00005:1).^4;
...
for ii = 1:length(zpRng)
    ...
    [~,Sol,f] = PullMembrane(alpha, mesh, lambda, acoat, rF(ii), rIn, zpRng(ii), f0, k0, gamma, C0(ii), R0, initSol);
...
plotTitle = sprintf(' \\lambda = %0.3f pN/nm,  P = %0.3f pN/nm^2, A_{coat} = %0.3f nm^2', lambda, P, acoat*2*pi*R0^2);
%xLim = [-20 20];
