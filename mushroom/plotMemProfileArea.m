...
function plotMemProfileArea(Sol, t, R0, coatArea, coatArea2, actArea, barArea, xLim, yLim, plotTitle, dash, varargin)
...
% plots the isotropic-curvature-generating coat2
if ~isempty(coatArea2)
    % loops over multiple regions
    for ii = 1:size(coatArea2,1)
        ...
        if dash == 1
            ...
            plot(Sol(1,t>=aMin & t<=aMax)*R0, Sol(2,t>=aMin & t<=aMax)*R0, ':', ...
                -Sol(1,t>=aMin & t<=aMax)*R0, Sol(2,t>=aMin & t<=aMax)*R0, ':', 'Color', coatColor2, 'LineWidth', lineWidth);
